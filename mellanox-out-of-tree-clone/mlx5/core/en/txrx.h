...
#define INL_HDR_START_SZ                                                       \
  (sizeof(((struct mlx5_wqe_eth_seg *)NULL)->inline_hdr.start))
...
#define MLX5E_MAX_TX_IPSEC_DS                                                  \
  DIV_ROUND_UP(sizeof(struct mlx5_wqe_inline_seg) + 255 + 1 + 1 + 16,          \
               MLX5_SEND_WQE_DS)
...
#define MLX5E_MAX_TX_INLINE_DS                                                 \
  DIV_ROUND_UP(366 - INL_HDR_START_SZ + VLAN_HLEN, MLX5_SEND_WQE_DS)

/* Sync the calculation with mlx5e_sq_calc_wqe_attr. */
#define MLX5E_MAX_TX_WQEBBS                                                    \
  DIV_ROUND_UP(MLX5E_TX_WQE_EMPTY_DS_COUNT + MLX5E_MAX_TX_INLINE_DS +          \
                   MLX5E_MAX_TX_IPSEC_DS + MAX_SKB_FRAGS + 1,                  \
               MLX5_SEND_WQEBB_NUM_DS)

#define MLX5E_RX_ERR_CQE(cqe) (get_cqe_opcode(cqe) != MLX5_CQE_RESP_SEND)

static inline ktime_t mlx5e_cqe_ts_to_ns(cqe_ts_to_ns func,
                                         struct mlx5_clock *clock, u64 cqe_ts) {
  return INDIRECT_CALL_2(func, mlx5_real_time_cyc2time,
                         mlx5_timecounter_cyc2time, clock, cqe_ts);
}
...
static inline bool mlx5e_skb_fifo_has_room(struct mlx5e_skb_fifo *fifo) {
  return (u16)(*fifo->pc - *fifo->cc) <= fifo->mask;
}

static inline bool mlx5e_wqc_has_room_for(struct mlx5_wq_cyc *wq, u16 cc,
                                          u16 pc, u16 n) {
  return (mlx5_wq_cyc_ctr2ix(wq, cc - pc) >= n) || (cc == pc);
}

static inline void *mlx5e_fetch_wqe(struct mlx5_wq_cyc *wq, u16 pi,
                                    size_t wqe_size) {
  void *wqe;
...
#define MLX5E_TX_FETCH_WQE(sq, pi)                                             \
  ((struct mlx5e_tx_wqe *)mlx5e_fetch_wqe(&(sq)->wq, pi,                       \
                                          sizeof(struct mlx5e_tx_wqe)))

static inline struct mlx5e_tx_wqe *mlx5e_post_nop(struct mlx5_wq_cyc *wq,
                                                  u32 sqn, u16 *pc) {
...
static inline struct mlx5e_tx_wqe *mlx5e_post_nop_fence(struct mlx5_wq_cyc *wq,
                                                        u32 sqn, u16 *pc) {
  u16 pi = mlx5_wq_cyc_ctr2ix(wq, *pc);
  struct mlx5e_tx_wqe *wqe = mlx5_wq_cyc_get_wqe(wq, pi);
  struct mlx5_wqe_ctrl_seg *cseg = &wqe->ctrl;
...
static inline u16 mlx5e_shampo_get_cqe_header_index(struct mlx5e_rq *rq,
                                                    struct mlx5_cqe64 *cqe) {
  return be16_to_cpu(cqe->shampo.header_entry_index) &
...
static inline void mlx5e_notify_hw(struct mlx5_wq_cyc *wq, u16 pc,
...
static inline struct mlx5e_sq_dma *mlx5e_dma_get(struct mlx5e_txqsq *sq,
                                                 u32 i) {
  return &sq->db.dma_fifo[i & sq->dma_fifo_mask];
}

static inline void mlx5e_dma_push(struct mlx5e_txqsq *sq, dma_addr_t addr,
                                  u32 size, enum mlx5e_dma_map_type map_type) {
...
static inline struct sk_buff **mlx5e_skb_fifo_get(struct mlx5e_skb_fifo *fifo,
                                                  u16 i) {
  return &fifo->fifo[i & fifo->mask];
}

static inline void mlx5e_skb_fifo_push(struct mlx5e_skb_fifo *fifo,
                                       struct sk_buff *skb) {
...
static inline struct sk_buff *mlx5e_skb_fifo_pop(struct mlx5e_skb_fifo *fifo) {
...
static inline void mlx5e_dump_error_cqe(struct mlx5e_cq *cq, u32 qn,
  ...
  netdev_err(cq->netdev,
             ...
             cq->mcq.cqn, ci, qn, get_cqe_opcode((struct mlx5_cqe64 *)err_cqe),
...
static inline void
mlx5e_eseg_swp_offsets_add_vlan(struct mlx5_wqe_eth_seg *eseg) {
...
static inline void mlx5e_set_eseg_swp(struct sk_buff *skb,
...
static inline u16 mlx5e_stop_room_for_wqe(struct mlx5_core_dev *mdev,
                                          u16 wqe_size) {
  WARN_ON_ONCE(PAGE_SIZE / MLX5_SEND_WQE_BB <
               (u16)mlx5e_get_max_sq_wqebbs(mdev));
  ...
  WARN_ONCE(wqe_size > mlx5e_get_max_sq_wqebbs(mdev),
            "wqe_size %u is greater than max SQ WQEBBs %u", wqe_size,
            mlx5e_get_max_sq_wqebbs(mdev));
...
static inline bool mlx5e_icosq_can_post_wqe(struct mlx5e_icosq *sq,
                                            u16 wqe_size) {
...
static inline struct mlx5e_mpw_info *mlx5e_get_mpw_info(struct mlx5e_rq *rq,
                                                        int i) {
  size_t isz = struct_size(rq->mpwqe.info, alloc_units.frag_pages,
